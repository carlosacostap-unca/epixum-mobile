...
const tools = [
  ...
  {
    name: "validate_mobile_exam_schema",
    description: "Check whether the partial exam and question bank collections expected by epixum-mobile exist.",
    inputSchema: {
      type: "object",
      properties: {},
      additionalProperties: false,
...
const requiredMobileExamCollections = [
  "partial_exams",
  "partial_exam_units",
...
const toolHandlers = {
  ...
  async validate_mobile_exam_schema() {
    const pb = await authenticate();
    ...
    const missing = requiredMobileExamCollections.filter((name) => !names.has(name));

    return {
      ok: missing.length === 0,
      required: requiredMobileExamCollections,
      missing,
      present: requiredMobileExamCollections.filter((name) => names.has(name)),
    };
  },
};
  ...
  switch (message.method) {
    case "initialize":
      return {
        ...
        serverInfo: {
          name: "mcp-pocketbase-epixum-mobile",
          version: "0.1.0",
        },
      };
      ...
      return {
        resources: [
          ...
          },
          {
            ...
            name: "Epixum Mobile partial exam schema status",
          ...
          contents: [
            {
              uri: "pocketbase://course-schema",
              mimeType: "application/json",
              text: JSON.stringify(await toolHandlers.validate_mobile_exam_schema(), null, 2),
            },
          ],
        };
      }
